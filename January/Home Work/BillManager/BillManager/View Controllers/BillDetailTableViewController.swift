...
class BillDetailTableViewController: UITableViewController, UITextFieldDelegate {
    ...
    func presentNeedAuthorizationAlert() {
        let alert = UIAlertController(title: "Authorization Needed you fetcher", message: "We can't set reminders for you without notification permissions, duh. Please go to the iOS Settings app and grant us notification permissions...pleb", preferredStyle: .alert)
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        if remindSwitch.isOn {
            bill.scheduleReminders(on: remindDatePicker.date) { (updatedBill) in
