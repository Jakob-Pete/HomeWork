...
func add(one: Int?, two: Int?) -> Int? {
    guard let one = one, let two = two, one != 0 && two != 0 else {
        return nil
    }
    return one + two
    }
...
createUser()
if let createUsr = createUser(){
    print("i'm \(createUsr.firstName), \(createUsr.lastName) and i am \(createUsr.age) years old")
 }
