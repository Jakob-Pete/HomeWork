...
func checkInventory(button: String) -> Double? {
    if let stock = stock[button] {
        if stock > 0 {
            if let price = prices[button] {
                return price
            } else {
                return nil
            }
...
print(checkInventory(button: "Apple"))
/*:
